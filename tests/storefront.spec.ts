...
async function unlock(page: Page) {
  ...
  await expect(page.getByRole('heading', { name: 'Find your building supplies.', exact: true })).toBeVisible();
}
...
test('search, category, brand, empty state and product detail page work', async ({ page }) => {
  await unlock(page);
  ...
  await page.getByRole('link', { name: 'View SikaGrout®-212 HP', exact: true }).click();
  await expect(page).toHaveURL(/#\/product\/SIKA212$/);
  await expect(page.getByRole('heading', { level: 1, name: 'SikaGrout®-212 HP', exact: true })).toBeVisible();
  await expect(page.getByRole('main')).toContainText('Sika Australia');
  ...
  await page.getByRole('link', { name: 'Masonry', exact: true }).click();
  await expect(page.getByRole('heading', { level: 1, name: 'Masonry', exact: true })).toBeVisible();
  await expect(page.locator('.product-card')).toHaveCount(Math.min(24, new Set(products.filter(p => p.category === 'Masonry').map(p => p.familyId)).size));
  await page.getByRole('link', { name: 'Clear filters', exact: true }).click();
  const filters = page.getByRole('button', { name: 'Filters & categories', exact: true });
  if (await filters.isVisible()) await filters.click();
  await page.getByRole('combobox', { name: 'Filter by brand', exact: true }).selectOption('TOPCON');
  await expect(page.locator('.product-card')).toHaveCount(new Set(products.filter(p => p.brand.toLowerCase() === 'topcon').map(p => p.familyId)).size);
...
test('layout fits narrow phones through desktop; branch choice persists', async ({ page }) => {
  ...
  await page.getByRole('combobox', { name: 'Your local branch', exact: true }).selectOption('Cairns');
  await page.reload();
  await expect(page.getByRole('combobox', { name: 'Your local branch', exact: true })).toHaveValue('Cairns');
});
test('contact draft survives browsing but is not stored and clears on reload', async ({ page }) => {
  ...
  expect(await page.evaluate(() => JSON.stringify({ local: localStorage, session: sessionStorage }))).not.toContain('Draft Builder');
...
test('unavailable product photos have an honest visible fallback', async ({ page }) => {
  ...
  await unlock(page);
  await page.getByRole('link', { name: 'View OneMix Concrete Mix', exact: true }).scrollIntoViewIfNeeded();
